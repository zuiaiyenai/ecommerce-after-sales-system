...
| 前端契约测试 | PASS | 17/17，包含管理端文本导入正式路由、管理员首页和账号/绩效视图无固定 Mock 数据契约 |
...
最终真实性复核又使用 `admin_demo` JWT 读取管理员 overview、客服账号和知识库接口：当前为 1 个启用客服、0 个待审批/未分配账号、51 条知识记录和 0 条需维护记录。管理员首页已删除固定商家、任务、治理进度和操作日志数据；账号治理页不再伪造设备、安全告警、知识权限和操作日志；绩效卡没有真实样本时显示空态，不再补造综合分、指标或近 7 日趋势。前端契约测试 17/17 和 real API 生产构建均通过。
