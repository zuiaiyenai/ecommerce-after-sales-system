...
class AfterSalesSupplementServiceImplTest {
    ...
    void persistsTicketEvidenceAndChatMessagesThenEnqueuesFormalReview() {
        ...
        ticket.setAiReviewRequestId("review-21");
        ticket.setEvidenceRevision(1);
        when(ticketMapper.selectByIdForUpdate(21L)).thenReturn(ticket);
        when(ticketMapper.selectById(21L)).thenReturn(ticket);
        when(ticketMapper.advanceEvidenceRevision(any(), any(), any(), any())).thenReturn(1);
        ...
        verify(reviewEvents).enqueueReviewResumed(
                ticket,
                12L,
                "补充商品破损照片",
                "supplement-001",
    ...
    void returnsExistingResultForRepeatedSupplementRequest() {
        ...
        existing.setEventType("after_sales.review.resume");
