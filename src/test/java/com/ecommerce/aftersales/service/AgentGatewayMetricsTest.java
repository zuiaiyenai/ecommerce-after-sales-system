...
class AgentGatewayMetricsTest {
    ...
    void publishesTheSameBusinessSignalsToMicrometer() {
        ...
        assertThat(registry.get("agent.gateway.capacity").gauge().value()).isEqualTo(2D);
        assertThat(registry.get("agent.gateway.inflight").gauge().value()).isEqualTo(1D);
        assertThat(registry.get("agent.gateway.sse.inflight").gauge().value()).isEqualTo(1D);
        metrics.sseFinished();
        metrics.permitReleased();
        assertThat(registry.get("agent.gateway.inflight").gauge().value()).isEqualTo(0D);
        assertThat(registry.get("agent.gateway.sse.inflight").gauge().value()).isEqualTo(0D);
    }
