...
class AfterSalesTicketMapperMySqlTest {
        ...
        try (var connection = ds.getConnection(); var statement = connection.createStatement()) {
            statement.execute("""
                    CREATE TABLE after_sales_ticket (
                      ...
                      ai_review_request_id VARCHAR(64) NULL UNIQUE, ai_review_result VARCHAR(30) NULL,
    ...
    @Test void onlyOneConcurrentAiUpdateCanApply() throws Exception {
        insert(1L, "PENDING");
        Callable<Integer> apply = () -> { try (SqlSession s = factory.openSession(true)) { return s.getMapper(AfterSalesTicketMapper.class).applyAiApprovalIfPending(1L, "r" + Thread.currentThread().threadId(), "ok", "{}", BigDecimal.ONE, LocalDateTime.now(), LocalDateTime.now()); } };
    ...
    @Test void humanAndAiRaceEndsWithHumanDecisionAndAiCannotOverwriteItLater() throws Exception {
        ...
        try (SqlSession s = factory.openSession(true)) {
            assertThat(s.getMapper(AfterSalesTicketMapper.class).applyAiApprovalIfPending(30L, "late-ai", "ok", "{}", BigDecimal.ONE, LocalDateTime.now(), LocalDateTime.now())).isZero();
        }
    }

    private static void insert(long id, String status) throws Exception { try (var c = MYSQL.createConnection("" ); var p = c.prepareStatement("insert into after_sales_ticket(id,ticket_no,order_id,order_no,user_id,reason,status) values(?,?,?,?,?,?,?)")) { p.setLong(1,id); p.setString(2,"T"+id); p.setLong(3,id); p.setString(4,"O"+id); p.setLong(5,id); p.setString(6,"OTHER"); p.setString(7,status); p.executeUpdate(); } }
