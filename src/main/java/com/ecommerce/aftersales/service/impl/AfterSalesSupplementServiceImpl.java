...
public class AfterSalesSupplementServiceImpl implements AfterSalesSupplementService {
    ...
    public SupplementAfterSalesResponse supplement(
        ...
        AfterSalesTicket ticket = ticketMapper.selectByIdForUpdate(ticketId);
        ...
        if (!StringUtils.hasText(ticket.getAiReviewRequestId())) {
            throw new BizException(409, "当前工单缺少有效的 AI 审批实例，请转人工处理");
        }
        ...
        if (existingEvent != null) {
            if (!ticketId.equals(existingEvent.getAggregateId())
                    || !"after_sales.review.resume".equals(existingEvent.getEventType())) {
                throw new BizException(409, "补充请求幂等键已被占用");
            }
            return response(ticket, request.getSessionId(), requestId, 0, true);
        }
        ...
        int advanced = ticketMapper.advanceEvidenceRevision(
                ticketId,
                ticket.getAiReviewRequestId(),
                "已收到补充材料，正在继续当前 AI 审批。",
                LocalDateTime.now()
        );
        if (advanced != 1) {
            throw new BizException(409, "当前 AI 审批状态已变化，请刷新工单状态");
        }
        ticket = ticketMapper.selectById(ticketId);
        reviewEventService.enqueueReviewResumed(
                ticket,
                session.getId(),
                message,
                requestId,
    ...
    private SupplementAfterSalesResponse response(
        ...
        response.setReviewRequestId(ticket.getAiReviewRequestId());
        response.setEvidenceRevision(ticket.getEvidenceRevision());
