...
public class AgentGatewayServiceImpl implements AgentGatewayService {
    ...
    void initializeConcurrencyGuard() {
        ...
        metrics.setGatewayCapacity(gatewayCapacity);
    ...
    public AgentGatewayDtos.ReviewImagesResponse reviewImages(AgentGatewayDtos.ReviewImagesRequest request) {
        ...
        try {
            permitAcquired = acquireAgentPermit();
            if (!permitAcquired) {
                outcome = "rejected";
                throw new BizException(429, "Agent 鏈嶅姟绻佸繖锛岃绋嬬瓑鍚庨噸璇");
            }
            AgentGatewayDtos.ReviewImagesResponse response = postJson(url, request, AgentGatewayDtos.ReviewImagesResponse.class);
            outcome = "success";
            return response;
            ...
            releaseAgentPermit(permitAcquired, startedAt, outcome);
        }
    }
    ...
    public SseEmitter streamChat(AgentGatewayDtos.ChatRequest request) {
        ...
        long startedAt = System.nanoTime();
        final boolean permitAcquired;
        try {
            permitAcquired = acquireAgentPermit();
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            metrics.recordSseRejected();
            metrics.record("rejected", 0);
            throw new BizException(429, "Agent 鏈嶅姟绻佸繖锛岃绋嬬瓑鍚庨噸璇");
        }
        if (!permitAcquired) {
            metrics.recordSseRejected();
        ...
        metrics.sseStarted();
        try {
            agentSseExecutor.execute(() -> {
                String outcome = "failure";
                try {
                    outcome = forwardAgentStream(request, emitter);
                } finally {
                    metrics.sseFinished();
                    releaseAgentPermit(true, startedAt, outcome);
                }
            });
        } catch (RejectedExecutionException exception) {
            metrics.sseFinished();
            releaseAgentPermit(permitAcquired, startedAt, "rejected");
            metrics.recordSseRejected();
            throw new BizException(429, "Agent SSE 鏈嶅姟绻佸繖锛岃绋嬬瓑鍚庨噸璇");
        }
        return emitter;
    }

    private String forwardAgentStream(AgentGatewayDtos.ChatRequest body, SseEmitter emitter) {
    ...
    private boolean acquireAgentPermit() throws InterruptedException {
        boolean acquired = agentRequestPermits.tryAcquire(
    ...
    private void releaseAgentPermit(boolean permitAcquired, long startedAt, String outcome) {
        if (!permitAcquired) {
            return;
        }
        metrics.record(outcome, Math.max(0, System.nanoTime() - startedAt));
        metrics.permitReleased();
        agentRequestPermits.release();
    }
