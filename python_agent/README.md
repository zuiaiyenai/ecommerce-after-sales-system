...
> 当前目录结构、依赖方向和唯一运行链路以 [ARCHITECTURE.md](ARCHITECTURE.md) 为准。Python Agent 通过 Java 内部工具访问业务数据，不直接写 MySQL 业务表。
...
The Agent uses provider-native Function Calling by default. `LLM_LEGACY_TOOL_CALL_FALLBACK_ENABLED=true`
keeps one temporary JSON-tool-decision retry for providers that reject or return an invalid native
tool call. Set it to `false` during strict-mode migration to fail closed instead of invoking the
...
- `after_sales_agent/api/http_server.py`
  - Spring Boot 调用的 Python Agent HTTP 入口，默认监听 `127.0.0.1:8000`
- `after_sales_agent/`
  - 售后 Agent 核心包
  - `domain_models.py`
    - 订单、售后请求、意图、状态、风险、图片审核等领域模型
  - `agents/`
    - 意图识别、状态机、凭证检查、风险评估、情绪识别、转人工判断等规则 Agent
  - `services/`
    - 对话编排、Qwen/Ollama 调用、图片审核、会话持久化等应用服务
  - `infra/`
    - MySQL 访问和请求 trace 等基础设施
  - `utils/`
    - 图片审核结果转换、序列化等共享工具
- `.env`
