...
from ..rag_query_rewrite import RagRewriteError
from ..rag_retrieval_policy import RagRetrievalPolicy
from ..tool_registry import ToolResult


class PolicySubagentRuntime(Protocol):
    def handle(self, task: PolicyTask) -> PolicyAssessment:
        ...


class EvidenceSubagentRuntime(Protocol):
    def handle(self, task: EvidenceTask) -> EvidenceAssessment:
        ...


class PolicyRewriteService(Protocol):
    def evaluate(self, **kwargs: Any) -> Any:
        ...


class PolicySubagentState(TypedDict, total=False):
    task: PolicyTask
...
class EvidenceSubagentState(TypedDict, total=False):
    task: EvidenceTask
...
@dataclass
class PolicySubagentGraph:
    port: PolicyRetrievalPort
    rewrite_service: Optional[PolicyRewriteService] = None
    _specialist: PolicySpecialist = field(init=False, repr=False)
    _retrieval_policy: RagRetrievalPolicy = field(init=False, repr=False)

    def __post_init__(self) -> None:
        self._specialist = PolicySpecialist(self.port)
        self._retrieval_policy = RagRetrievalPolicy()
        graph = StateGraph(PolicySubagentState)
    ...
    def handle(self, task: PolicyTask) -> PolicyAssessment:
        ...
        if not isinstance(assessment, PolicyAssessment):
            raise RuntimeError("policy subagent did not produce an assessment")
        return assessment

    @staticmethod
    def receive_task(state: PolicySubagentState) -> PolicySubagentState:
        task = state["task"]
        missing = [
            name
            for name, value in (
    ...
    def retrieve_policy(
        self,
        state: PolicySubagentState,
    ) -> PolicySubagentState:
        return {
            "retrieval_result": self._specialist.retrieve(state["task"]),
    ...
    def observe_policy_result(
        ...
        assessment = self._specialist.assess(state["task"], result)
        ...
        if (
            ...
            try:
                decision = self.rewrite_service.evaluate(
                    ...
                    task="评估正式售后审核所需的可信政策覆盖，只改写检索问题，不作业务决定。",
                    known_facts={
    ...
    def retrieve_policy_multi(
        ...
        return {
            "retrieval_result": self._specialist.retrieve_multi(
                state["task"],
...
@dataclass
class EvidenceSubagentGraph:
    port: EvidenceReviewPort
    _specialist: EvidenceSpecialist = field(init=False, repr=False)

    def __post_init__(self) -> None:
        self._specialist = EvidenceSpecialist(self.port)
        graph = StateGraph(EvidenceSubagentState)
        graph.add_node("receive_task", self.receive_task)
    ...
    def handle(self, task: EvidenceTask) -> EvidenceAssessment:
        ...
        if not isinstance(assessment, EvidenceAssessment):
            raise RuntimeError("evidence subagent did not produce an assessment")
        return assessment

    @staticmethod
    def receive_task(state: EvidenceSubagentState) -> EvidenceSubagentState:
        task = state["task"]
        missing = [
            name
            for name, value in (
    ...
    def review_evidence(
        self,
        state: EvidenceSubagentState,
    ) -> EvidenceSubagentState:
        return {
            "image_review_result": self._specialist.review(state["task"]),
            "steps": int(state.get("steps") or 0) + 1,
        }
