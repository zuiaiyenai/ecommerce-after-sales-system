...
from ...infra.request_tracing import TraceRecorder, bind_trace_id
...
class AgenticRagChatService:
    ...
    def _handle_core(
        ...
        with self._trace_step(
            ...
            reply = self._generate_answer(payload, knowledge, trusted_hits)
        append_arguments = {
    ...
    def _trusted_hits(knowledge: dict[str, Any]) -> list[dict[str, Any]]:
        if (
            ...
            or knowledge.get("reranker_succeeded") is not True
            or str(knowledge.get("mode") or "")
            not in {"hybrid_reranked", "pgvector", "multi_query_reranked"}
        ):
            return []
        return [
            dict(hit)
            ...
            and AgenticRagChatService._citations([hit])
        ]
