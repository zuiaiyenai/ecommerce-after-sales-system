...
from ...domain.models import AfterSalesRequest, ConversationMessage, EmotionAnalysisResult, EmotionLabel, Order
...
# Union type alias for the two backend result types
EmotionBackendResult = Union[QwenEmotionResult, LLMEmotionResult]


@dataclass(frozen=True)
class EmotionAgent:
    backend: Union[QwenEmotionBackend, LLMEmotionBackend] = field(
        default_factory=QwenEmotionBackend,
    )
    ...
    def analyze(
        ...
        *,
        ...
        backend_result = self.backend.analyze(
            request,
            order,
            recent_history=recent_history,
            recent_user_messages=recent_user_messages,
        )
        label = self._map_label(backend_result.label)
        strategy = DEFAULT_EMOTION_STRATEGIES.get(label.value, DEFAULT_EMOTION_STRATEGIES["calm"])
        triggers = self._build_triggers(backend_result)
        need_human_priority = self._resolve_human_priority(
            label,
            handoff_min_level=handoff_min_level,
            llm_need_human_priority=backend_result.need_human_priority,
        )

        return EmotionAnalysisResult(
            label=label,
            score=max(0.0, min(1.0, float(backend_result.emotion_score))),
            confidence=max(0.0, min(1.0, float(backend_result.confidence))),
            triggers=triggers,
    ...
    @staticmethod
    def _build_triggers(result: EmotionBackendResult) -> tuple[str, ...]:
        backend_name = type(result).__name__.lower()
        triggers = [f"{backend_name}_emotion"]
        if getattr(result, "reason", ""):
            triggers.append(f"{backend_name}_reasoned")
