...
def _traceable_citations(hit: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(citations, list):
        ...
        citations = [citation] if isinstance(citation, dict) else []
    if not isinstance(citations, list):
        return []
    return [
        dict(item)
...
class PolicyEvaluator:
    ...
    def assess(self, task: PolicyTask, result: ToolResult) -> PolicyAssessment:
        ...
        retrieval_mode = str(knowledge.get("mode") or "")
        ...
        for hit in hits:
            ...
            if retrieval_mode != "multi_query_rrf":
                hit_score = _score(
        ...
        return PolicyAssessment(
            ...
            policy_match_score=max(
                ...
                default=(1.0 if mode == "multi_query_rrf" and trusted_hits else 0.0),
            )
            if mode != "multi_query_rrf"
            else (1.0 if trusted_hits else 0.0),
