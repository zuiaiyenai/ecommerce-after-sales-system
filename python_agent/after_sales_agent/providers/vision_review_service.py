...
class VisionReviewService:
    PROMPT_VERSION: ClassVar[str] = "2026-07-11-damage-confidence-v1"
    ...
    def _review_single_attachment(
        ...
        *,
        ...
        cache_key = self._cache_key(attachment.source or "", order_hint)
    ...
    def _cache_key(self, source: str, order_hint: str | None) -> str:
        ...
        try:
            with sqlite3.connect(path, timeout=2) as connection:
                ...
                if row is None or time.time() - float(row[0]) > ttl:
                    ...
                    return None
                item = ImageReviewItem(**json.loads(str(row[1])))
