...
from after_sales_agent.interface.http_adapter import AfterSalesHttpAdapter
from after_sales_agent.application.chat import (
    analyze_chat_emotion,
    build_attachments,
    build_recent_history_messages,
)
from after_sales_agent.application.knowledge_admin_service import KnowledgeAdminService
from after_sales_agent.application.knowledge_ingestion import KnowledgeParseError
from after_sales_agent.application.request_payload_adapter import (
    build_chat_entry_payload,
    build_recent_history_payload,
)
...
from after_sales_agent.domain.models import ImageReviewResult
from after_sales_agent.infrastructure import AGENT_RUNTIME_METRICS, TRACE_HISTORY, record_trace_event
...
HTTP_ADAPTER = AfterSalesHttpAdapter(AFTER_SALES_AGENT)
VISION_SERVICE = VisionReviewService()
STREAMING_CHAT_SERVICE = StreamingChatService(
    chat_service=AFTER_SALES_AGENT,
)
...
class AgentApiHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        with bind_trace_id(self._trace_id()):
            self._do_post_traced()

    def _do_post_traced(self) -> None:
        if not self._ensure_internal_authorized():
            return
        try:
    ...
    def _handle_chat_stream(self) -> None:
        trace = TraceRecorder(request_type="chat_stream", trace_id=self._trace_id())
        with trace.step("read_request_body"):
            data = self._read_json_body()
        message = str(data.get("message") or "").strip()
        if not message:
            self._send_json({"error": "message_required"}, status=400)
            trace.set_meta(error="message_required")
            record_trace_event(trace, path="/api/chat/stream")
            return
        ...
        request = StreamingChatRequest(
            ...
            payload=build_chat_entry_payload(data),
        )
        self.send_response(200)
        self._send_cors_headers()
        ...
        self.end_headers()
        try:
            with trace.step("agentic_rag_chat"):
                for event in STREAMING_CHAT_SERVICE.stream(
                    request,
                    trace_recorder=trace,
                ):
                    self._write_sse(event)
        except (BrokenPipeError, ConnectionAbortedError, ConnectionResetError, socket.error):
            trace.set_meta(outcome="client_disconnected")
            logging.getLogger("api_server.stream").info("stream_client_disconnected request_id=%s", request.request_id)
        except Exception as exc:
            trace.set_meta(outcome="error", error=exc.__class__.__name__)
            ...
            if "outcome" not in trace.meta:
                trace.set_meta(outcome="completed")
            record_trace_event(
                trace,
    ...
    def _handle_chat(self) -> None:
        ...
        try:
            with trace.step("agentic_rag_chat"):
                result = HTTP_ADAPTER.handle_chat(
                    build_chat_entry_payload(data, emotion_context),
                    trace_recorder=trace,
                )
        except Exception as exc:
            logging.getLogger("api_server").exception("❌ agentic RAG chat 执行异常: %s", exc)
            trace.set_meta(
                error=exc.__class__.__name__,
        ...
        payload = {
            ...
            "suggested_action": "agentic_rag_chat",
