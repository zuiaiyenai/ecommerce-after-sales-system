...
from after_sales_agent.interface.http_server import build_attachments
...
class NativeFunctionCallingStrictWorkflowTest(unittest.TestCase):
    @staticmethod
    ...
    def test_native_function_disabled_fails_closed_without_model_call(self) -> None:
    ...
    def test_native_terminal_failure_fails_closed_without_second_protocol(self) -> None:
        llm = SwitchingDecisionLlm(
        ...
        self.assertIn("当前智能售后服务暂时不可用", result["assistant_reply"])
        self.assertEqual(1, llm.native_calls)
        self.assertEqual(0, llm.legacy_calls)
        self.assertEqual("fail_closed", state["decision_protocol"])

    def test_native_function_disabled_failure_trace_is_deterministic(self) -> None:
        tools = NativeWorkflowTools()
        llm = SwitchingDecisionLlm(
            [],
        ...
        trace = state["tool_results"][-1]
        self.assertEqual("deterministic", trace["function_call_mode"])
        self.assertEqual("none", trace["provider_tool_call_shape"])

    def test_native_handoff_tool_call_routes_once_through_dedicated_node(self) -> None:
        cases = {
            "confirmed": (
                NativeWorkflowTools(),
                "HUMAN",
        ...
        for name, (tools, expected_mode, succeeded, transport_ok, error_type) in cases.items():
            with self.subTest(name=name):
                ...
                self.assertEqual([], agent.handoff_input["evidence_needed"])
                self.assertEqual("call-handoff", agent.handoff_input.get("pending_tool_call_id"))
                ...
                self.assertEqual("native", trace["function_call_mode"])
                self.assertEqual("openai", trace["provider_tool_call_shape"])
                self.assertEqual("call-handoff", trace["tool_call_id"])
                if not succeeded:
                    self.assertNotIn(
                        "已为您转接人工客服，请稍等",
    ...
    def test_native_protocol_failure_fails_closed_without_legacy_action(self) -> None:
    ...
    def test_invalid_native_call_does_not_execute_legacy_tool(self) -> None:
        tools = NativeWorkflowTools()
        llm = SwitchingDecisionLlm(
            [native_response("call-unknown", "unknown_tool", {})],
            [{"action": "tool_call", "tool_name": "lookup", "tool_arguments": {}}],
    ...
    def test_native_protocol_failure_cannot_submit_review_from_non_kafka_source(self) -> None:
        tools = NativeWorkflowTools()
        llm = SwitchingDecisionLlm(
            [native_response("call-unknown", "unknown_tool", {})],
    ...
    def test_terminal_decider_ignores_legacy_reply_after_native_failure(self) -> None:
        llm = SwitchingDecisionLlm(
            [native_response("call-unknown", "unknown_tool", {})],
        ...
        self.assertEqual(0, llm.legacy_calls)
        self.assertEqual("fail_closed", state["decision_protocol"])
        self.assertIn("当前智能售后服务暂时不可用", action["assistant_reply"])
    ...
    def test_terminal_handle_decision_matrix_preserves_protocol_boundaries(self) -> None:
        ...
        for name, case in cases.items():
            with self.subTest(name=name):
                ...
                self.assertEqual(case["lookup_calls"], tools.calls.count("lookup"))
                self.assertNotIn("submit_ai_review", tools.calls)
                self.assertEqual(["append_chat_message", "append_chat_message"], tools.calls[-2:])
                if case["lookup_calls"]:
