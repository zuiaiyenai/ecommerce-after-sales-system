...
from after_sales_agent.interface.kafka_adapter import (
...
class KafkaReviewConsumerTest(unittest.TestCase):
    ...
    def test_agent_failure_submits_manual_fallback_with_same_review_request_id(self) -> None:
        ...
        self.assertEqual(0, payload["evidenceRevision"])
        self.assertNotIn("expectedEvidenceRevision", payload)
    ...
    def test_missing_required_fields_goes_to_invalid_message_dlq_shape(self) -> None:
        ...
        self.assertEqual("", dlq["review_request_id"])
    ...
    def test_dlq_is_confirmed_before_failed_state_and_offset_commit(self) -> None:
        ...
        payload = (
            b'{"event_id":"evt-run","review_request_id":"review-run","ticket_id":"10001","user_id":"20001",'
            b'"order_id":"30001"}'
        )
    ...
    def test_fresh_processing_is_deferred_and_only_terminal_state_is_committed(self) -> None:
        events: list[object] = []
        payload = (
            b'{"event_id":"evt-processing","review_request_id":"review-processing","ticket_id":"10001","user_id":"20001",'
            b'"order_id":"30001"}'
        )
        ...
        with patch.dict(sys.modules, {"kafka": kafka_module}), patch(
            "after_sales_agent.interface.kafka_adapter.time.sleep"
        ):
            consumer.run_forever()
    ...
    def test_metrics_record_review_duration_and_bounded_outcome_labels(self) -> None:
        metrics = ConsumerMetrics()
        started_at = metrics.start_review()
        metrics.finish_review(started_at - 0.01, "completed")
        metrics.finish_review(metrics.start_review(), "unexpected-dynamic-outcome")

        snapshot = metrics.snapshot()
        self.assertEqual(1, snapshot["review_processing_total"]["completed"])
        self.assertEqual(1, snapshot["review_processing_total"]["failed"])
        self.assertEqual(2, snapshot["review_processing_duration_ms"]["count"])
        self.assertGreater(snapshot["review_processing_duration_ms"]["max"], 0)
        self.assertEqual(0, snapshot["review_inflight"])

    def test_metrics_prometheus_payload_has_fixed_labels(self) -> None:
        metrics = ConsumerMetrics()
        started_at = metrics.start_review()
        metrics.finish_review(started_at, "completed")

        payload = metrics.prometheus_payload().decode("utf-8")
        self.assertIn('after_sales_review_processing_total{outcome="completed"} 1', payload)
        self.assertIn("after_sales_review_processing_duration_ms_count 1", payload)
        self.assertNotIn("event-123", payload)
