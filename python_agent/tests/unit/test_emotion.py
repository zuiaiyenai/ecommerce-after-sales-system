...
from after_sales_agent.application.emotion.llm_emotion_classifier import LLMEmotionBackend, LLMEmotionResult
from after_sales_agent.application.emotion.qwen_emotion_backend import (
    QwenEmotionBackend,
    QwenEmotionResult,
)
...
class LLMEmotionBackendTest(unittest.TestCase):
    def test_self_check_no_longer_ties_calm_to_handoff_priority(self) -> None:
        ...
        self.assertFalse(LLMEmotionBackend._needs_self_check(result))

    def test_score_ranges_are_more_conservative(self) -> None:
        self.assertEqual((0.16, 0.35), LLMEmotionBackend.SCORE_RANGES["calm"])
        self.assertEqual((0.30, 0.55), LLMEmotionBackend.SCORE_RANGES["anxious"])
        self.assertEqual((0.50, 0.75), LLMEmotionBackend.SCORE_RANGES["dissatisfied"])
    ...
    def test_normalize_result_keeps_in_range_values(self) -> None:
        ...
        normalized = LLMEmotionBackend._normalize_result(result)
        self.assertEqual("dissatisfied", normalized.label)
        self.assertEqual(0.52, normalized.emotion_score)
        self.assertEqual(0.98, normalized.confidence)
    ...
    def test_needs_self_check_when_score_exceeds_one(self) -> None:
        ...
        self.assertTrue(LLMEmotionBackend._needs_self_check(result))
    ...
    def test_label_from_score_maps_satisfied(self) -> None:
        self.assertEqual("satisfied", LLMEmotionBackend._label_from_score(0.05))

    def test_label_from_score_maps_calm(self) -> None:
        self.assertEqual("calm", LLMEmotionBackend._label_from_score(0.20))

    def test_label_from_score_maps_anxious(self) -> None:
        self.assertEqual("anxious", LLMEmotionBackend._label_from_score(0.40))

    def test_label_from_score_maps_dissatisfied(self) -> None:
        self.assertEqual("dissatisfied", LLMEmotionBackend._label_from_score(0.60))

    def test_label_from_score_maps_angry(self) -> None:
        self.assertEqual("angry", LLMEmotionBackend._label_from_score(0.88))

    def test_parse_required_bool_supports_yes_and_no(self) -> None:
        self.assertTrue(LLMEmotionBackend._parse_required_bool("yes", "need_human_priority"))
        self.assertFalse(LLMEmotionBackend._parse_required_bool("no", "need_human_priority"))

    def test_parse_required_bool_rejects_invalid_text(self) -> None:
        with self.assertRaises(Exception):
            LLMEmotionBackend._parse_required_bool("maybe", "need_human_priority")
